...
exports.isValidHL7 = isValidHL7;
/**
 * Service de conversion HL7 vers FHIR
 */
const simple_hl7_1 = __importDefault(require("simple-hl7"));
const uuid_1 = require("uuid");
// Configuration du parseur HL7
const parser = new simple_hl7_1.default.Parser();
/**
 ...
 */
function processMessageHeader(segment, fhirBundle) {
    const messageHeader = {
 ...
 */
function processPatientIdentification(segment, fhirBundle) {
    const patientId = segment.getField(3).toString() || (0, uuid_1.v4)();
    const patient = {
 ...
 */
function processPatientVisit(segment, fhirBundle) {
    const encounter = {
        ...
        subject: {
            reference: `Patient/${fhirBundle.entry.find((e) => e.resource.resourceType === 'Patient')?.resource.id || 'unknown'}`
        },
        period: {
 ...
 */
function processObservationRequest(segment, fhirBundle) {
    const serviceRequest = {
        ...
        encounter: {
            reference: `Encounter/${fhirBundle.entry.find((e) => e.resource.resourceType === 'Encounter')?.resource.id || 'unknown'}`
        },
        code: {
            coding: [
                {
 ...
 */
function processObservationResult(segment, fhirBundle) {
    const observation = {
...
function parseHL7Date(hl7Date) {
    ...
    try {
        const date = new Date(parseInt(year, 10), parseInt(month, 10) - 1, // Les mois sont indexés à partir de 0 en JS
