...
router.post('/', apiKeyAuth_1.apiKeyAuth, (req, res) => {
    try {
        ...
        const application = (0, database_1.getApplicationById)(parseInt(applicationId));
        if (!application) {
        ...
        // Créer la clé API
        const apiKey = (0, apiKeyService_1.createNewApiKey)(parseInt(applicationId));
 ...
 *         description: Erreur serveur
 */
router.get('/application/:applicationId', apiKeyAuth_1.apiKeyAuth, (req, res) => {
    try {
        ...
        // Vérifier que l'application existe
        const application = (0, database_1.getApplicationById)(applicationId);
        if (!application) {
            return res.status(404).json({
                success: false,
                error: 'Not Found',
        ...
        const apiKeys = (0, apiKeyService_1.getApiKeysForApplication)(applicationId);
...
router.get('/:id', apiKeyAuth_1.apiKeyAuth, (req, res) => {
    try {
        ...
        const apiKey = (0, apiKeyService_1.getApiKeyDetails)(id);
        if (!apiKey) {
            return res.status(404).json({
                success: false,
                error: 'Not Found',
 ...
 *         description: Erreur serveur
 */
router.post('/:id/revoke', apiKeyAuth_1.apiKeyAuth, (req, res) => {
    try {
        const id = parseInt(req.params.id);
        if (isNaN(id)) {
            return res.status(400).json({
        ...
        // Révoquer la clé API
        const revoked = (0, apiKeyService_1.revokeExistingApiKey)(id);
        if (!revoked) {
            return res.status(500).json({
                success: false,
                error: 'Internal Server Error',
